...
foreach ($nas_disk_conf_dir in Get-ChildItem "$conf_dir\nas.conf") {
  ...
  if (Test-Path -Path $nas_srv_conf_file -PathType Leaf) { 
    ...
    $nas_servers.add( @{net_backup_dir = "$($nas_srv_conf[0])"; dokdc = $nas_srv_conf[1]; dokmc = $nas_srv_conf[2]; include_db_to = $include_db_to_nas; exclude_db_to = $exclude_db_to_nas})
  } else {
    $nas_srv_conf = ""
