...
# Версия 2.5.3
# От 15.08.2025
# Добавлена процедура вычисления свободного места и отправки  этих данных в Telegram
# От 08.08.2025
# При копировании обрабатываются только файлы за последние сутки (условик в цикле)
# От 06.08.2025
...
# Оповещать о свободном месте на диске
$is_about_condition_disks = $true
#============================================
  ...
  try {
    Get-ChildItem "$pWork_Dir" | where { $_.LastAccessTime -lt ((Get-Date).AddDays(-$pdok)) } | %{ 
      try {
        ...
        write-host "INFO: Удаление файла $pWork_Dir\$_"
      } catch {
        $err_message  = "`n$($PSItem.Exception.Message)"
        write-host "ERRO: Ошибка удаления файла $pWork_Dir\$_"
      }
    }
  } catch {
    $err_message  = "`n$($PSItem.Exception.Message)"
...
function Delete-DBArchive-in-Dir-All {
  ...
  $err_message = (Delete-DBArchive-in-Dir "$tmp_log_dir" $($doktc * 2))

  if ($err_message -eq "") {
    $message = $message + "`nУдаление файлов из папки $($tmp_log_dir) завершено успешно."
  } else {
    $message = $message + "`nУдаление файлов из папки $($tmp_log_dir) завершено с ошибками:"
    $err_message_all = $err_message_all + $err_message
  }
  ...
  if ($is_save_nas) {
    ...
    foreach ($nas_server in $nas_servers) {
      ...
      if ($nas_server.include_db_to.Count -eq 0) {
        ...
        foreach ($file_name in Get-ChildItem "$tmp_backup_dir" | where { $_.LastAccessTime -ge ((Get-Date).AddDays(-1)) }) {
...
#--------------------------------------------
# Функция оповещения о состоянии дисков
#--------------------------------------------
function Warning-About-Condition-Disks {
  if ($is_about_condition_disks) {
    $message  = "<b>" + $srv_name + "</b>"

    write-host "---                Вычисление доступного места на дисках                 ---"
    foreach ($cdrive in Get-WmiObject Win32_LogicalDisk | Select-Object DeviceID, FreeSpace, Size) {
      if ($cdrive.DeviceID -ne $null) {
        write-host "INFO: Диск $($cdrive.DeviceID) свободно $([Math]::Round($($cdrive.FreeSpace /1GB),2)) из $([Math]::Round($($cdrive.Size /1GB),2)) (GB)"
        $message = $message + "`nДиск $($cdrive.DeviceID) свободно $([Math]::Round($($cdrive.FreeSpace /1GB),2)) из $([Math]::Round($($cdrive.Size /1GB),2)) (GB)"
      }
    }

    $Response = (Send-Telegram $message "")
  }
}
#============================================
# Основное тело скрипта
#============================================
# Оповещение о состоянии дисков
$err_message = (Warning-About-Condition-Disks)
